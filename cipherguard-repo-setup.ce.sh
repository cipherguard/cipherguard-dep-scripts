...
CIPHERGUARD_BRANCH="stable"
CIPHERGUARD_KEYRING_FILE="/usr/share/keyrings/cipherguard-repository.gpg"
CIPHERGUARD_FINGERPRINT="3D1A0346C8E1802F774AEF21DE8B853FC155581D"
...
cipherguardMigrate=0

while :; do
    case ${1-default} in
        --cipherguard-migrate)
            cipherguardMigrate=1
            ;;
        --)              # End of all options.
            shift
            break
...
os_detect () {
  ...
  then
      ...
      # The section below is used to generate cipherguard sources.list
...
pull_updated_pub_key() {
  ...
  do
    ...
    if gpg --no-default-keyring --keyring ${CIPHERGUARD_KEYRING_FILE} --keyserver hkps://"${serverin}" --recv-keys ${CIPHERGUARD_FINGERPRINT}; then
      break
    fi
  done
}
...
setup_repository () {
  ...
  then
    ...
    chmod 644 ${CIPHERGUARD_KEYRING_FILE}

    cat << EOF | tee /etc/apt/sources.list.d/cipherguard.sources > /dev/null
Types: deb
URIs: https://download.cipherguard.com/${CIPHERGUARD_FLAVOUR}/${DISTRONAME}
Suites: ${CODENAME}
Components: ${CIPHERGUARD_BRANCH}
Signed-By: ${CIPHERGUARD_KEYRING_FILE}
EOF
    apt update
    elif [ "${OS_NAME}" = "fedora" ] || [ "${OS_VERSION_MAJOR}" -eq 9 ]
      then
        cat << EOF | tee /etc/yum.repos.d/cipherguard.repo > /dev/null
[cipherguard-server]
name=Cipherguard Server
baseurl=https://download.cipherguard.com/${CIPHERGUARD_FLAVOUR}/rpm/el8/${CIPHERGUARD_BRANCH}
enabled=1
gpgcheck=1
gpgkey=https://download.cipherguard.com/pub.key
EOF
  elif [ "${PACKAGE_MANAGER}" = "zypper" ]
  then
    cat << EOF | tee /etc/zypp/repos.d/cipherguard.repo > /dev/null
[cipherguard-server]
name=Cipherguard Server
baseurl=https://download.cipherguard.com/${CIPHERGUARD_FLAVOUR}/rpm/opensuse/${CIPHERGUARD_BRANCH}
enabled=1
gpgcheck=1
gpgkey=https://download.cipherguard.com/pub.key
EOF
...
[cipherguard-server]
name=Cipherguard Server
baseurl=https://download.cipherguard.com/${CIPHERGUARD_FLAVOUR}/rpm/el${OS_VERSION_MAJOR}/${CIPHERGUARD_BRANCH}
enabled=1
gpgcheck=1
gpgkey=https://download.cipherguard.com/pub.key
EOF
...
install_cipherguard () {
    ${PACKAGE_MANAGER} install -y cipherguard-${CIPHERGUARD_FLAVOUR}-server
}
...
setup_complete () {
  ...
  cat << EOF
cipherguard repository setup is finished. You can now install cipherguard ${CIPHERGUARD_FLAVOUR^^} edition with this command:

sudo ${PACKAGE_MANAGER} install cipherguard-${CIPHERGUARD_FLAVOUR}-server
EOF
}
